...
def execute(filters=None):
    ...
    data = get_m_entries(filters)

    balances = get_entries(filters)
...
def get_m_entries(filters):
    journal_entries = frappe.db.sql("""
		...
		where jvd.parent = jv.name and jv.docstatus=1
			...
			and ifnull(jv.clearance_date, '4000-01-01') > %(from_date)s
    ...
    payment_entries = frappe.db.sql("""
		...
		where
			...
			and ifnull(clearance_date, '4000-01-01') > %(from_date)s
	""", filters, as_dict=1)
